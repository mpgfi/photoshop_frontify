...
(function () {
    ...
    function splitTextLayer(layer) {
        ...
        // Group consecutive paragraphs that share the same font into one text box.
        // Only cut at font-family boundaries so that e.g. two same-font lines
        // stay together instead of being split into separate layers.
        var groups     = [];
        var charOffset = 0;
        for (var i = 0; i < lines.length; i++) {
            ...
            if (lineLen > 0) {
                ...
                var gFont = (sr && sr.font)  ? sr.font  : defaultFont;
                var gCol  = (sr && sr.color) ? sr.color : null;
                if (groups.length > 0 && groups[groups.length - 1].font === gFont) {
                    groups[groups.length - 1].lines.push(lineText);
                } else {
                    groups.push({ font: gFont, color: gCol, lines: [lineText] });
                }
            }
            charOffset += lineLen + 1;
        }

        var totalLinesSoFar = 0;
        for (var g = 0; g < groups.length; g++) {
            var grp       = groups[g];
            var groupText = grp.lines.join('\r');

            var dup = layer.duplicate(layer, ElementPlacement.PLACEBEFORE);
            var dti = dup.textItem;

            try { dti.contents = groupText; } catch (e) {}
            if (grp.font)  { try { dti.font  = grp.font;  } catch (e) {} }
            if (grp.color) { try { dti.color = grp.color; } catch (e) {} }
            try { dti.hyphenation = false; } catch (e) {}

            // Shift down by the accumulated height of all preceding groups.
            if (totalLinesSoFar > 0) {
                try { dup.translate(0, totalLinesSoFar * singleLineH); } catch (e) {}
            }

            // Count the visual lines this group occupies and accumulate.
            try {
                var dupH        = dup.bounds[3].value - dup.bounds[1].value;
                var linesInGrp  = Math.round(dupH / singleLineH);
                if (linesInGrp < 1) linesInGrp = 1;
                totalLinesSoFar += linesInGrp;
            } catch (e) { totalLinesSoFar += grp.lines.length; }

            // Shrink-to-fit via autoSizePolicy.
            selectLayer(dup);
            try {
